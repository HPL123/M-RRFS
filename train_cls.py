...
from mmdetection.splits import get_seen_class_ids, get_class_labels
...
class TrainCls():
    ...
    def initDataSet(self, features, labels):
        ...
        self.dataloader = DataLoader(dataset=self.dataset, batch_size=self.opt.batch_size_cls, num_workers=4, shuffle=True, pin_memory=True)
        self.test_dataloader = DataLoader(dataset=self.test_dataset, batch_size=self.opt.batch_size_cls * 50, num_workers=4, shuffle=True, pin_memory=True)
...
#     def __init__(self, opt):
#         # self.classes_to_train = np.concatenate(([0], get_unseen_class_ids(opt.dataset, split=opt.classes_split)))
#         self.classes_to_train = np.concatenate(([0], get_seen_class_ids(opt.dataset, split=opt.classes_split),get_unseen_class_ids(opt.dataset, split=opt.classes_split)))
#         self.opt = opt
#         # self.classes = get_unseen_class_labels(self.opt.dataset, split=opt.classes_split)
#         self.classes = get_class_labels(self.opt.dataset)
#         self.best_acc = -100000
#         self.isBestIter = False
#         self.criterion = nn.CrossEntropyLoss()
#         # self.criterion = nn.NLLLoss()
