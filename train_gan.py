...
class TrainGAN():
    def __init__(self, opt, attributes, unseenAtt, unseenLabels, memory, unseen_att_with_bg, unseen_att_labels_with_bg, seen_feats_mean, gen_type='FG'):
        ...
        self.unseen_labels = unseenLabels
        self.memory = memory
        self.unseen_att_with_bg = unseen_att_with_bg
        # self.unseen_att_labels_with_bg = unseen_att_labels_with_bg
        self.unseen_att_labels_with_bg = np.array([i for i, l in enumerate(unseen_att_labels_with_bg)])

        self.Wu_Labels_un = np.array([i for i, l in enumerate(unseenLabels)])
        ...
        # self.opt.tau = 1
        # self.opt.radius = 0.0003
        # self.opt.num_negative = 10
        # self.opt.featnorm = True
        # inter contra
        # inter contra
    ...
    def generate_clus_syn_feature(self, labels, attribute, num=100, no_grad=True):
        """
        generates features
        inputs:
            labels: features labels to generate nx1 n is number of objects
        ...
        # syn_feature = torch.FloatTensor(nclass * num, self.opt.resSize)
        # syn_label = torch.LongTensor(nclass * num)
        syn_feature = torch.FloatTensor(nclass * num, self.opt.resSize).cuda()
        syn_label = torch.LongTensor(nclass * num).cuda()
        ##

        ##
        # syn_att = torch.FloatTensor(num, self.opt.attSize)
        # syn_noise = torch.FloatTensor(num, self.opt.nz)
        syn_att = torch.FloatTensor(num, self.opt.attSize).cuda()
        syn_noise = torch.FloatTensor(num, self.opt.nz).cuda()
        ##
    ...
    def trainEpoch(self):
        ...
        for i in range(0, self.ntrain, self.opt.batch_size):
            ...
            # input_att = torch.sigmoid(input_att)
            ...
            #if self.epoch > self.opt.first_stage_epoch:
            fake_unseen_f, fake_unseen_l = self.generate_clus_syn_feature(self.unseen_labels, self.Wu,
                                                                      num=self.opt.batch_size // 4, no_grad=False)
            
            clus_input_features = torch.cat((fake1, fake_unseen_f), 0)
            clus_input_labels = torch.cat((input_label, fake_unseen_l), 0)
            #
            clus_contra_loss = self.memory(clus_input_features, clus_input_labels)
            
            cluster_contra_loss = self.opt.lambda_clus_contra * clus_contra_loss
            ...
            ##
            # errG = -G_cost - G_cost2 + c_errG + loss_lz + fake_inter_contras_loss + unseenc_errG + loss_lz_su

            # if self.epoch < self.opt.uncls_dely:
            #     errG = -G_cost - G_cost2 + c_errG + loss_lz
            # else:
            #     errG = -G_cost - G_cost2 + c_errG + loss_lz + unseenc_errG

            # if self.epoch > self.opt.first_stage_epoch:
            errG = -G_cost - G_cost2 + c_errG + loss_lz + fake_inter_contras_loss + cluster_contra_loss
            ...
            print(f"{self.gen_type} [{self.epoch+1:02}/{self.opt.nepoch:02}] [{i:06}/{int(self.ntrain)}] \
                                                Loss: {errG.item() :0.4f} D loss: {D_cost.data.item():.4f} G loss: {G_cost.data.item():.4f}, W dist: {Wasserstein_D.data.item():.4f} \
                                                 seen loss: {c_errG.data.item():.4f} div: {loss_lz.item():0.4f} real_inter_contras_loss: {real_inter_contras_loss.data.item():.4f} fake_inter_contras_loss : {fake_inter_contras_loss.data.item():.4f} cluster_contra_loss : {cluster_contra_loss.item():.4f}")
            # else:
            # print(f"{self.gen_type} [{self.epoch+1:02}/{self.opt.nepoch:02}] [{i:06}/{int(self.ntrain)}] \
            #                  Loss: {errG.item() :0.4f} D loss: {D_cost.data.item():.4f} G loss: {G_cost.data.item():.4f}, W dist: {Wasserstein_D.data.item():.4f} \
            #                  seen loss: {c_errG.data.item():.4f} div: {loss_lz.item():0.4f} real_inter_contras_loss: {real_inter_contras_loss.data.item():.4f} fake_inter_contras_loss : {fake_inter_contras_loss.data.item():.4f}")
