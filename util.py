...
#         if ite % 100 == 99:
#             print(
#                 f'{verbose}   Epoch [{epoch+1:02}/{opt.nepoch_cls}] Iter [{ite:05}/{len(dataloader)}]{ite/len(dataloader) * 100:02.3f}% Loss: {loss_epoch/ite :0.4f}')
#
#     preds_all = np.concatenate(preds_all)
#     gt_all = np.concatenate(gt_all)
#     return compute_per_class_acc(gt_all, preds_all, opt, verbose=verbose)
...
#     own_dict['fc1.bias'].copy_(state_dict['bbox_head.fc_cls.bias'].cuda())
#
#     assert (model.state_dict()['fc1.bias'] == state_dict[
#         'bbox_head.fc_cls.bias'].cuda()).all(), 'Something wrong with loading pretrained fasterrcnn cls head!!'
#     print(f"loaded classifier from {filepath}")
#     return model
